-- Table: netflix_apollo_graphql.access_tracking

-- DROP TABLE IF EXISTS netflix_apollo_graphql.access_tracking;

CREATE TABLE IF NOT EXISTS netflix_apollo_graphql.access_tracking
(
...
ALTER TABLE IF EXISTS netflix_apollo_graphql.access_tracking
    OWNER to postgres;

COMMENT ON TABLE netflix_apollo_graphql.access_tracking
    IS 'Provides history on object access and operation.';

COMMENT ON COLUMN netflix_apollo_graphql.access_tracking.accessor_id
    IS 'The fully qualified id of a user or system process.';

COMMENT ON COLUMN netflix_apollo_graphql.access_tracking.object_type
    IS 'A fully qualified object type, provided by the tracking system';

COMMENT ON COLUMN netflix_apollo_graphql.access_tracking.operation
...
-- DROP INDEX IF EXISTS netflix_apollo_graphql.access_tracking_accessor_id;

CREATE INDEX IF NOT EXISTS access_tracking_accessor_id
    ON netflix_apollo_graphql.access_tracking USING btree
    (accessor_id COLLATE pg_catalog."default" ASC NULLS LAST)
    TABLESPACE pg_default;
-- Index: access_tracking_accessor_operations

-- DROP INDEX IF EXISTS netflix_apollo_graphql.access_tracking_accessor_operations;
...
COMMENT ON INDEX netflix_apollo_graphql.access_tracking_accessor_operations
    IS 'Provides optimized access based on who did what, to what type, and when';
-- Index: access_tracking_object_type_operation

-- DROP INDEX IF EXISTS netflix_apollo_graphql.access_tracking_object_type_operation;
...
-- Table: netflix_apollo_graphql.saved_objects

-- DROP TABLE IF EXISTS netflix_apollo_graphql.saved_objects;

CREATE TABLE IF NOT EXISTS netflix_apollo_graphql.saved_objects
(
    id uuid NOT NULL DEFAULT gen_random_uuid(),
...
ALTER TABLE IF EXISTS netflix_apollo_graphql.saved_objects
    OWNER to postgres;

COMMENT ON TABLE netflix_apollo_graphql.saved_objects
    IS 'Provides history on object access and operation.';

COMMENT ON COLUMN netflix_apollo_graphql.saved_objects.object_type
    IS 'A fully qualified object type, provided by the saving system';

COMMENT ON COLUMN netflix_apollo_graphql.saved_objects.object_id
    IS 'A fully qualified object id, provided by the saving system';
-- Index: saved_objects_created_by_object

-- DROP INDEX IF EXISTS netflix_apollo_graphql.saved_objects_created_by_object;

CREATE UNIQUE INDEX IF NOT EXISTS saved_objects_created_by_object
    ON netflix_apollo_graphql.saved_objects USING btree
